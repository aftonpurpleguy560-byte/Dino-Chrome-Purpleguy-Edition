...
const DinoGame = () => {
    ...
    const loop = () => {
      ...
      g.obstacles.forEach((o, i) => {
        ...
        if (g.dino.x < o.x + 20 && g.dino.x + 30 > o.x && g.dino.y < o.y + 40 && g.dino.y + 40 > o.y) {
          ...
          addDoc(collection(db, "dino_leaderboard"), { name: "Efe", score, timestamp: serverTimestamp(), signature: "Purpleguy © 2026 - tablet power" });
        }
  ...
  return (
    <div className="game-container" onClick={() => gameState === 'PLAYING' && gameRef.current.dino.y === 150 && (gameRef.current.dino.dy = -12)}>
        ...
        {gameState !== 'PLAYING' && (
          <div className="menu-overlay">
            ...
            <button onClick={(e) => { e.stopPropagation(); start(); }} className="neon-btn">
              {gameState === 'MENU' ? 'OYUNA BAŞLA' : 'REKORU KIR'}
            </button>
            ...
            <div className="leaderboard-card">
              ...
              {leaderboard.map((l, i) => (
                <div key={i} className="leaderboard-item">
                  ...
                  <span className="font-bold text-white">{l.score}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <footer className="footer-signature">Purpleguy © 2026 - tablet power</footer>
    </div>
  );
};
